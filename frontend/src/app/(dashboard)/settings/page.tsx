...
export default function WorkspaceSettings() {
  ...
  const handleToggle2Fa = async () => {
    if (twoFaEnabled) {
      ...
      try {
        ...
        setQrCodeUrl(res.data.data.otpauth_url || res.data.data.qrCodeUrl);
        setShow2FaModal(true);
      } catch (e: any) {
        toast.error(e.response?.data?.error || "Failed to initiate 2FA");
      } finally {
        setTwoFaLoading(false);
      }
    }
  ...
  const handleVerify2FaCode = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await api.post("/auth/2fa/verify", { code: verificationCode, totp_code: verificationCode });
      ...
      toast.error(e.response?.data?.error || "Invalid 2FA code");
    } finally {
      setTwoFaLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="rounded-xl border border-border bg-card p-6 rounded-2xl border border-border space-y-4">
          ...
          <form onSubmit={handleUpdateProfile} className="space-y-4">
            <div>
              ...
              <input
                ...
                className="w-full px-3.5 py-2.5 bg-background border border-border rounded-xl focus:outline-none focus:ring-2 focus:ring-primary text-sm text-foreground"
              />
            </div>
            <div>
            ...
            <button
              ...
              className="px-4 py-2.5 rounded-xl bg-primary text-primary-foreground font-semibold hover:bg-primary/95 text-xs disabled:opacity-50"
            >
            ...
            <form onSubmit={handleUpdateOrg} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3.5 py-2.5 bg-white/2 border border-border rounded-xl text-sm text-muted-foreground font-mono"
                />
              </div>
              <button
                type="submit"
        ...
        <div className="rounded-xl border border-border bg-card p-6 rounded-2xl border border-border space-y-4 lg:col-span-2">
          <h3 className="text-sm font-bold text-foreground flex items-center gap-2">
            <KeyRound className="w-4.5 h-4.5 text-primary" /> Security & 2-Factor Auth
          </h3>
          
          <div className="p-4 rounded-xl bg-white/2 border border-border flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div className="space-y-1">
              <p className="text-xs font-bold text-foreground">Authenticator App Verification (TOTP)</p>
      ...
      {show2FaModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="rounded-xl border border-border bg-card w-full max-w-sm rounded-2xl border border-border p-6 z-10 shadow-2xl relative text-center">
            <button
            ...
            <form onSubmit={handleVerify2FaCode} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3.5 py-2.5 bg-background border border-border rounded-xl focus:outline-none focus:ring-2 focus:ring-primary text-center tracking-widest font-bold text-sm text-foreground"
                />
              </div>
              ...
              <div className="flex items-center justify-end gap-3 pt-4 border-t border-border">
                <button
                  type="button"
                  onClick={() => setShow2FaModal(false)}
                  className="px-4 py-2 text-xs font-semibold rounded-xl bg-background border border-border text-foreground hover:bg-white/10"
                >
                  Cancel
                </button>
                <button
      ...
      <div className="rounded-xl border border-border bg-card p-6">
        <h2 className="text-sm font-semibold">Developer — Mock WhatsApp</h2>
        <p className="mt-1 text-sm text-muted-foreground">Inject a real inbound message through the API (not frontend fake data).</p>
        <MockInbound />
      </div>
    </div>
  );
...
function MockInbound() {
  ...
  const [body, setBody] = useState("Hello from mock WhatsApp");
  return (
    <div className="mt-4 flex flex-wrap gap-2">
      <input className="h-9 rounded-lg border border-input px-3 text-sm" value={phone} onChange={(e) => setPhone(e.target.value)} />
      <input className="h-9 flex-1 rounded-lg border border-input px-3 text-sm" value={body} onChange={(e) => setBody(e.target.value)} />
      <button
        className="h-9 rounded-lg bg-primary px-3 text-sm text-primary-foreground"
        onClick={async () => {
          try {
            const { api } = await import("@/lib/api");
            await api.post("/dev/mock/inbound", { phone_number: phone, body, contact_name: "Mock User" });
            toast.success("Inbound message created");
          } catch (e: any) {
            toast.error(e.response?.data?.error || "Mock inbound failed");
          }
        }}
      >
        Simulate inbound
